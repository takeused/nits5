          ...
          <div style="background:#f8fafc;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px;padding:20px 24px;font-size:12px;color:#374151;line-height:1.7;">
            ...
            <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px;">
              ...
              <div style="background:white;border:1px solid #e5e7eb;border-left:4px solid #059669;border-radius:6px;padding:12px 14px;">
                ...
                <p style="margin:0;color:#6b7280;">논문 20건 미만, 핵심 데이터 오류, 신뢰도 60점 미만 후보는 정식 순위에서 보류합니다. 논문 5~19건 후보는 성장·신뢰도 조건을 만족할 때만 <strong>초기 탐색 후보</strong>로 별도 표시합니다. 통과 후보는 <strong>공백 매력도</strong>(연구 기반 35% + IP 공백 신호 40% + 연구 모멘텀·안정성 25%), <strong>전환·실행 근거</strong>(NTIS·보고서·특허 전환 신호), <strong>데이터 신뢰도</strong>(API 상태·검색 완화 깊이·관측량·추세 완결성)로 분리합니다. 직접 IP 공백은 <strong>log10((논문+1)/(특허+1))/2</strong>로 산출합니다. <strong>로그를 취하는 이유</strong>는 ① 논문·특허 건수가 분야마다 수십 건~수만 건으로 자릿수 자체가 달라, 원 비율을 그대로 쓰면 대형 분야가 점수를 독식하기 때문이고 ② 공백의 의미는 <em>차이</em>가 아니라 <em>배수</em>이기 때문입니다 — 논문이 특허의 10배인 상황은 100건/10건이든 10,000건/1,000건이든 동일한 공백 신호여야 하는데, 로그는 이 배수 관계를 같은 간격으로 변환해 규모 편향을 제거합니다(1억→10억과 10억→100억이 같은 간격이 되는 것과 같은 원리). ③ 분자·분모에 <strong>+1</strong>을 더하는 것은 특허 0건일 때 값이 무한대로 발산하는 것을 막고, 표본이 극소수인 후보가 만점을 받지 않게 하기 위함입니다. 마지막에 <strong>÷2</strong>로 나누는 것은 100배 차이(log10=2)를 공백 만점 1.0에 대응시켜 0~1 범위로 정규화하는 과정입니다. 후보군 상대 비교는 핵심 데이터가 정상이고 논문 5건 이상이며 검색 완화 수준·검색어 폭이 비슷한 후보가 3개 이상일 때만 사용하고, 후보 수와 분포 안정성에 따라 반영 비중을 낮춥니다.</p>
              </div>
